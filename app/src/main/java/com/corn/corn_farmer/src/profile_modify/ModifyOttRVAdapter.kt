...
import com.corn.corn_farmer.src.profile.model.ProfileOtt
import com.corn.cornfarmer_android.databinding.ItemProfileOttBinding

class ModifyOttRVAdapter(private val ottList : List<ProfileOtt>) : RecyclerView.Adapter<ModifyOttRVAdapter.Viewholder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ModifyOttRVAdapter.Viewholder {
        val binding : ItemProfileOttBinding = ItemProfileOttBinding.inflate(LayoutInflater.from(parent.context),parent,false)
        return Viewholder(binding)
    }

    override fun onBindViewHolder(holder: ModifyOttRVAdapter.Viewholder, position: Int) {
